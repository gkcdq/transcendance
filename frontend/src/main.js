...
function initModeGame(p1Name = "Player", p2Name = "IA")
{
    const name = userStore.get('user_name');
...
function startGameModeLogic(name1, name2, canvas, ctx)
{
    ...
    let leftPaddleY = canvas.width / 2 - (Math.min(canvas.width/2, canvas.height/2) - 10);
    let rightPaddleX = canvas.width - paddleWidth;
    let rightPaddleY = (canvas.height - p1paddleHeight) / 2;

    let ballX = canvas.width / 2, ballY = canvas.height / 2;
    let ballSpeedX = 5, ballSpeedY = 5;
    let score1 = 0, score2 = 0;

    // ── Files de bonus (max 3 chacun) ────────────────────────────────────────
    let p1Bonuses = [];
    let p2Bonuses = [];
    ...
    function octagonBounce() {
        ...
        // 1. Murs droits (Haut, Bas, Gauche, Droite)
        ...
        // 2. Murs inclinés (Coins) - On utilise une équation de ligne simple : x + y = constante
        // Coin Haut-Gauche (x + y < offset)
        if (ballX + ballY < offset) {
            let temp = ballSpeedX; ballSpeedX = Math.abs(ballSpeedY); ballSpeedY = Math.abs(temp);
            ballX = offset - ballY + 1;
        }
        // Coin Haut-Droit ((w - x) + y < offset)
        if ((w - ballX) + ballY < offset) {
            let temp = ballSpeedX; ballSpeedX = -Math.abs(ballSpeedY); ballSpeedY = Math.abs(temp);
            ballX = w - (offset - ballY) - 1;
        }
        // Coin Bas-Gauche (x + (h - y) < offset)
        if (ballX + (h - ballY) < offset) {
            let temp = ballSpeedX; ballSpeedX = Math.abs(ballSpeedY); ballSpeedY = -Math.abs(temp);
            ballX = offset - (h - ballY) + 1;
        }
        // Coin Bas-Droit ((w - ballX) + (h - ballY) < offset)
        if ((w - ballX) + (h - ballY) < offset) {
            let temp = ballSpeedX; ballSpeedX = -Math.abs(ballSpeedY); ballSpeedY = -Math.abs(temp);
            ballX = w - (offset - (h - ballY)) - 1;
        }
    }
    function constrainPaddle(px, py, pHeight) {
        const offset = canvas.width * 0.3;
        const w = canvas.width;
        const h = canvas.height;

        // Calcul de la limite Y min et max à la position X donnée
        // Pour le haut : si x < offset, y doit être > offset - x
        let minY = 0;
        if (px < offset) minY = offset - px;
        else if (px + paddleWidth > w - offset) minY = px + paddleWidth - (w - offset);

        // Pour le bas : si x < offset, y + height doit être < h - (offset - x)
        let maxY = h - pHeight;
        if (px < offset) maxY = h - pHeight - (offset - px);
        else if (px + paddleWidth > w - offset) maxY = h - pHeight - (px + paddleWidth - (w - offset));
    ...
    function update() {
        ...
        if (keys['w'] && leftPaddleY > 0)                             leftPaddleY -= speed;
        if (keys['s'] && leftPaddleY < canvas.height - p1paddleHeight)  leftPaddleY += speed;
        if (keys['d'] && leftPaddleX < PADDLE_MAX_X)                  leftPaddleX += hSpeed;
        if (keys['a'] && leftPaddleX > 0)                             leftPaddleX -= hSpeed;
        ...
        if (name2 === "IA") {
            ...
            // Contraint la raquette IA au bord droit de l'octogone
            const cx = canvas.width / 2, cy = canvas.height / 2;
            const r  = Math.min(cx, cy) - 10;
            rightPaddleX = cx + r - paddleWidth; // bord droit fixe
            // Contraint Y dans l'octogone
            rightPaddleY = Math.max(cy - r + 10, Math.min(cy + r - p2paddleHeight - 10, rightPaddleY));
        } else {
            const sp2 = 7 * p2SpeedMult, hs2 = 4 * p2SpeedMult;
            if (keys['ArrowUp']    && rightPaddleY > 0)                            rightPaddleY -= sp2;
            if (keys['ArrowDown']  && rightPaddleY < canvas.height - p2paddleHeight) rightPaddleY += sp2;
        ...
        if (ballSpeedX > 0) {
            const prevBallX = ballX - ballSpeedX;
            if (prevBallX < rightPaddleX + paddleWidth && ballX >= rightPaddleX) {
                if (ballY + 6 > rightPaddleY && ballY - 6 < rightPaddleY + p1paddleHeight) {
                    ballX = rightPaddleX - 1;
                    ballSpeedX = -Math.min(Math.abs(ballSpeedX) * 1.1, maxSpeed);
                }
            }
        ...
        if (ballSpeedX < 0) {
            const prevBallX = ballX - ballSpeedX;
            if (prevBallX > leftPaddleX && ballX <= leftPaddleX + paddleWidth) {
                if (ballY + 6 > leftPaddleY && ballY - 6 < leftPaddleY + p2paddleHeight) {
                    ballX = leftPaddleX + paddleWidth + 1;
                    ballSpeedX = Math.min(Math.abs(ballSpeedX) * 1.1, maxSpeed);
                }
            }
    ...
    function resetBall() {
        ...
        // Repositionner les raquettes aux bords horizontaux de l'octogone
        leftPaddleX = 10; 
    ...
    function draw() {
        ...
        // --- BORDURES HAUT ET BAS (Violet) ---
        ctx.strokeStyle = '#a855f7';
        ctx.shadowColor = '#a855f7';
        ctx.beginPath();
        ctx.moveTo(offset, 0); ctx.lineTo(w - offset, 0);
        ctx.moveTo(offset, h); ctx.lineTo(w - offset, h);
        ctx.stroke();
